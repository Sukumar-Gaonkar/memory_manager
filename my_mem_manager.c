...
//	pte *curr_shared_pg = (void *) memory + TOTAL_PAGES * PAGE_SIZE;
	...
	while(invt_pg_table[shared_pg_index].max_free < size && shared_pg_index < end_pg_index){
		shared_pg_index++;
	}
	...
	if(invt_pg_table[shared_pg_index].max_free >= size){
		...
		if(itr_addr->is_max_free_block == 1){
			...
			while(curr_offset < PAGE_SIZE){
				if(temp_addr->free == 1 && temp_addr->size > invt_pg_table[shared_pg_index].max_free){
...
//		return malloc(size);
		...
		if(th_pg_tb[tid] == NULL){
			...
			if(old_pg == -1){
				...
				for(;testBit(invt_swap_tb, swap_index) && swap_index < SWAP_PAGES; swap_index++);
				...
				if(write_in_swap(th_pg_tb[tid]->mem_page_no, swap_index) != -1){
					printf("Cant write in Swap File!!!\n");
					return NULL;
				}

				int old_tid = invt_pg_table[0].tid;
				th_pg_tb[old_tid]->in_memory = 0;
				th_pg_tb[old_tid]->swap_page_no = swap_index;
...
//				page->size = size;
				...
				th_pg_tb[tid]->in_memory = 1;
				th_pg_tb[tid]->mem_page_no = 0;
				th_pg_tb[tid]->next = NULL;
